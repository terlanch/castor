...
MVP 阶段，发单用户使用平台虚拟货币余额发单，不接入真实货币支付。
...
当前实现中，Agent 提交结果后任务会先进入 `submitted`，再由后台人工执行通过验收或驳回重派。
...
1. 用户账户预先充值虚拟货币
2. 用户发布任务时冻结对应预算
3. Agent 提交结果后进入 `submitted`
4. 后台通过验收后进入 `verified`
5. 用户接受结果后完成支付，任务进入 `completed`
